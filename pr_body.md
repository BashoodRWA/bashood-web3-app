...
Comando sugerido para crear el PR draft con GitHub CLI (ejecutar en la raíz del repo):

```bash
gh pr create --draft --base main --head feature/oracles-structure \
  --title "feat: scaffold Chainlink oracles (adapter + mocks skeleton)" \
  --body-file ./pr_body.md
```

Si no tenéis `gh`, abrid el PR por la UI: New pull request → compare `feature/oracles-structure` → Create draft pull request.

---
Hecho: el archivo `pr_body.md` está en la rama `feature/oracles-structure` y listo para usar como cuerpo del PR.
