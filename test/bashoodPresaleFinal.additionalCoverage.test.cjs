...
async function generateSignature(signer, userAddress, nonce) {
  ...
  describe("💰 ClaimProjectFunds Edge Cases", function () {
    it("Debe permitir claimProjectFunds con balance > 0", async function () {
      ...
      // claimProjectFunds debe ser llamado por el projectWallet (treasury)
    ...
    it("Debe revertir claimProjectFunds si no es admin", async function () {
      const { presale, user1 } = await loadFixture(deployPresaleFixture);
