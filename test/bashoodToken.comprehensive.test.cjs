...
describe("BashoodToken - Comprehensive Coverage Tests", function () {
  ...
  describe("⏸️ Pause/Unpause", function () {
    it("Debe pausar el contrato correctamente", async function () {
      const { token } = await loadFixture(deployBashoodTokenFixture);
      await token.pause();
    ...
    it("Debe despausar el contrato correctamente", async function () {
      const { token, user1 } = await loadFixture(deployBashoodTokenFixture);
      await token.pause();
      await token.unpause();
    ...
    it("Debe revertir pause si no es owner", async function () {
      const { token, user1 } = await loadFixture(deployBashoodTokenFixture);
      await expect(
    ...
    it("Debe revertir unpause si no es owner", async function () {
      const { token, user1 } = await loadFixture(deployBashoodTokenFixture);
      await token.pause();
      await expect(
