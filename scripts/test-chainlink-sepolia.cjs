...
const ORACLES = {
    ...
    "BTC/USD": "0x0FB99723Aee6f420beAD13e6bBB79b7E6F034298", // Si está disponible
    "USDC/USD": "0xd30e2101a97dcbAeBCBC04F14C3f624E67A35165", // Si está disponible
};
...
async function testOracle(oracleName, oracleAddress, ChainlinkPriceFeed, AggregatorV3Interface, deployer) {
    ...
    conSECURITY TEST 3: Historical Round Data Validation
    ...
    for (let i = 0; i < 10; i++) {
        ...
        try {
            ...
            console.log(`   Round ${targetRound}: ❌ Not available (${err.message.slice(0, 50)}...)`);
        }
    }
    
    logTest(`Oracle:${oracleName}`, "Historical rounds validation", "PASS",
        { validRounds: validHistoricalRounds, invalidRounds: invalidHistoricalRounds });onsole.log(`   🕐 Updated At: ${new Date(Number(updatedAt) * 1000).toISOString()}`);
    console.log(`   🔢 Answered In Round: ${answeredInRound}`);
    
    // CRITICAL TEST: answeredInRound >= roundId
    const isValid = answeredInRound >= roundId;
    console.log(`   ${isValid ? '✅' : '❌'} answeredInRound >= roundId: ${answeredInRound} >= ${roundId} = ${isValid}\n`);

    if (!isValid) {
        console.log("⚠️  WARNING: Oracle appears stale! answeredInRound < roundId");
        console.log("   This is the validation that was failing in local tests.\n");
    }

    // Historical rounds
    console.log("📊 Reading last 5 rounds (historical data)...");
    for (let i = 0; i < 5; i++) {
        const targetRound = BigInt(roundId) - BigInt(i);
        try {
            const [rid, ans, , upd, air] = await aggregator.getRoundData(targetRound);
            ...
            console.log(
                `   Round ${rid}: $${priceStr.padEnd(10)} | ` +
                `answered=${air.toString().padEnd(10)} | ` +
    ...
    try {
        ...
        try {
            ...
            if (error.message.includes("stale")) {
                console.log(`      ✅ Correctly rejected stale data: ${error.message.slice(0, 60)}...`);
    ...
    try {
        ...
        try {
            ...
            console.log(`      ⚠️  First call failed: ${error.message.slice(0, 60)}...`);
    ...
    for (let i = 0; i < 3; i++) {
        try {
            ...
            console.log(`   Call ${i + 1}: ❌ Failed - ${error.message.slice(0, 60)}...`);
        }
    }
...
function generateReport() {
    ...
    console.log("=".repeat(80) + "\n"
        console.log("\n🎉 SUCCESS! The answeredInRound >= roundId validation works with real Chainlink oracle!");
        console.log("   This confirms the issue was with MockPriceFeed, not ChainlinkPriceFeed.sol");
    }
    
    console.log("\n📍 Verify on Basescan:");
    console.log(`   https://sepolia.basescan.org/address/${priceFeedAddress}`);
    console.log("\n📍 Oracle Dashboard:");
    console.log(`   https://data.chain.link/base/testnet/crypto-usd/eth-usd`);
}
