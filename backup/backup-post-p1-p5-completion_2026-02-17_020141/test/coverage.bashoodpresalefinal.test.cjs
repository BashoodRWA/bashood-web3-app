...
describe('Coverage: BashoodPresaleFinal extra focused tests', function () {
  it.skip('submitProposal: burns via fallback transferFrom when burnFrom not available - DEPRECATED: function removed', async function () {
  ...
  it.skip('submitProposal: reverts when price is stale - DEPRECATED: function removed', async function () {
    // Funciones reactivadas para mejorar coverage
    const [owner, alice, project] = await ethers.getSigners();
    const MockBHT = await ethers.getContractFactory('contracts/mocks/MockBHT.sol:MockBHT');
    const bht = await MockBHT.deploy(); await bht.waitForDeployment();
  ...
  it.skip('delegate rescue: handles success and revert-with-reason and revert-without-reason - DEPRECATED: delegate* removed', async function () {
    // Funciones reactivadas para mejorar coverage
    const [owner, alice, project] = await ethers.getSigners();
    const MockBHT = await ethers.getContractFactory('contracts/mocks/MockBHT.sol:MockBHT');
    const bht = await MockBHT.deploy(); await bht.waitForDeployment();
